...
export function Square({ value, isWinning, isClickable, onClick }: SquareProps) {
  ...
  const interactiveClasses = !value && isClickable
    ? 'bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 cursor-pointer active:scale-90'
    : 'bg-slate-200 dark:bg-slate-700 cursor-default opacity-90';

  const winningClasses = isWinning ? 'ring-4 ring-emerald-500 bg-emerald-100/40 dark:bg-emerald-900/40' : '';
