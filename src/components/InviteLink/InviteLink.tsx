...
export function InviteLink({ gameId, variant = 'full' }: InviteLinkProps) {
  ...
  return (
    <div className="bg-slate-900 p-3 rounded-lg border border-slate-700 flex items-center gap-2 mb-6 overflow-hidden">
      <code className="text-xs text-indigo-300 truncate flex-1">{inviteLink}</code>
      <button
        onClick={handleCopy}
